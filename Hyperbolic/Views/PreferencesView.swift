...
struct PreferencesView: View {
        ...
        Section(languageService.s("other")) {
            ...
            if showNotifications {
                Button(languageService.s("test_notification")) {
                    NotificationService.shared.sendDownloadCompleted(filename: "Hyperbolic Test", languageService: languageService)
                }
                .buttonStyle(.link)
                .controlSize(.small)
            }
    ...
    private var aboutTab: some View {
        ScrollView {
            VStack(spacing: 16) {
                ...
                HStack(spacing: 16) {
                    Link(destination: URL(string: "https://github.com/offsideai/Hyperbolic")!) {
    ...
    private func updateYtdlp() {
        ...
        Task {
            ...
            let hyperbolicDir = appSupport.appendingPathComponent("Hyperbolic")
            let destination = hyperbolicDir.appendingPathComponent("yt-dlp")
            
            do {
                try FileManager.default.createDirectory(at: hyperbolicDir, withIntermediateDirectories: true)
